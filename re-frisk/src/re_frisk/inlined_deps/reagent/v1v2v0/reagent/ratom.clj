(ns ^{:mranderson/inlined true} re-frisk.inlined-deps.reagent.v1v2v0.reagent.ratom
  (:refer-clojure :exclude [run!])
  (:require [re-frisk.inlined-deps.reagent.v1v2v0.reagent.debug :as d]
            [re-frisk.inlined-deps.reagent.v1v2v0.reagent.interop :as interop]))

;; Note: this macro is duplicated in re-frisk.inlined-deps.reagent.v1v2v0.reagent.core,
;; with a docstring.
(defmacro reaction [& body]
  `(re-frisk.inlined-deps.reagent.v1v2v0.reagent.ratom/make-reaction
    (fn [] ~@body)))

(defmacro run!
  "Runs body immediately, and runs again whenever atoms deferenced in the body change. Body should side effect."
  [& body]
  `(let [co# (re-frisk.inlined-deps.reagent.v1v2v0.reagent.ratom/make-reaction (fn [] ~@body)
                                         :auto-run true)]
     (deref co#)
     co#))
...
(defmacro with-let [bindings & body]
  ...
  (let [v (with-meta (gensym "with-let") {:tag 'clj})
        ...
        add-destroy (when destroy
                      (list
                        `(let [destroy# ~destroy]
                           (if (re-frisk.inlined-deps.reagent.v1v2v0.reagent.ratom/reactive?)
                             (when (nil? (.-destroy ~v))
                               (set! (.-destroy ~v) destroy#))
                             (destroy#)))))
        asserting (if *assert* true false)
        res (gensym "res")]
    `(let [~v (re-frisk.inlined-deps.reagent.v1v2v0.reagent.ratom/with-let-values ~k)]
       ~(when asserting
          `(when-some [^clj c# re-frisk.inlined-deps.reagent.v1v2v0.reagent.ratom/*ratom-context*]
